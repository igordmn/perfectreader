...
object UserSettingValues {
    ...
    val TEXT_SCALE_X = floatArrayOf(
            ...
            1.0F, 1.05F, 1.1F, 1.15F, 1.2F, 1.25F, 1.3F, 1.35F, 1.4F, 1.45F, 1.5F, 1.6F, 1.7F, 1.8F, 1.9F, 2.0F
    )
...
fun scaleSettingValue(values: FloatArray, value: Float, offset: Int): Float = when {
    offset < 0 -> {
        val lowerIndex = values.binarySearchLower(value)
        if (lowerIndex >= 0) {
            val offsetIndex = max(0, lowerIndex + offset + 1)
            values[offsetIndex]
        } else {
            value
        }
    ...
    offset > 0 -> {
        ...
        if (greaterIndex <= values.size - 1) {
            val offsetIndex = min(values.size - 1, greaterIndex + offset - 1)
            values[offsetIndex]
        } else {
            value
        }
