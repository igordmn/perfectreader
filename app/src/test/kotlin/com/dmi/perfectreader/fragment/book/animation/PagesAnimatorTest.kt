...
import com.dmi.perfectreader.fragment.book.animation.PagesAnimator.Config
import com.dmi.test.shouldNearThreePrecision
import com.dmi.util.android.system.nanoSeconds
import org.junit.Test

@Suppress("IllegalIdentifier")
class PagesAnimatorTest {
    val config = Config(singlePageNanoTime = nanoSeconds(10.0))
    ...
    inner class Tester(seconds: Double, page: Double) {
        private var animator = PagesAnimator.zero(config, nanoSeconds(seconds)).currentPage(page).targetPage(page)

        fun update(seconds: Double) = doAction { update(nanoSeconds(seconds)) }
        fun addVelocity(velocity: Double) = doAction { velocity(this.velocity + velocity) }
        fun goNextPage() = doAction { targetPage(this.targetPage + 1) }
        fun goPreviousPage() = doAction { targetPage(this.targetPage - 1) }

        fun doAction(action: PagesAnimator.() -> PagesAnimator): PagesAnimator {
