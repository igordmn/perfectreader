...
class BookLocationsTest {
    ...
    fun `find nearest to list of ranged items`() {
        ...
        fun nearestTo(intLocation: Int): IntRange {
            val index = ranges.indexOfNearest({ this }, location(intLocation))
    ...
    fun range(intRange: IntRange) = BookRange(
            location(intRange.first),
            location(intRange.last)
    )

    fun intRange(range: BookRange) = range.begin.offset.toInt()..range.end.offset.toInt()

    fun location(intLocation: Int) = BookLocation(intLocation.toDouble())
}